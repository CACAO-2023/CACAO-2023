	...
	public double offre(Feve f, double cours_de_f) {
		if (f==Feve.F_BQ) {
			...
			if (this.Rentabilites(f, cours_de_f)==true && cours_de_f < prix_seuil_1) { //si le cours permet de faire au moins 10% de profit, on ne met en vente que les fèves 
				return this.getStockTotTimeTheo(Feve.F_BQ, (int)this.stepsVecuesPourBourseBQ.getValeur()); //BQ qui vont bientôt disparaître
			}
			if (cours_de_f >= prix_seuil_1 && cours_de_f <= prix_seuil_2) { // si le prix est suffisemment élevé, on met en vente aussi les fèves de MQ qui ne sont pas proches d'être déclassées
				return (stock_mis_en_bourse(f)-this.getStockTotTimeTheo(Feve.F_BQ, (int)this.stepsVecuesPourBourseBQ.getValeur()))*(cours_de_f - prix_seuil_1)/(prix_seuil_2 - prix_seuil_1) + this.getStockTotTimeTheo(Feve.F_BQ, (int)this.stepsVecuesPourBourseBQ.getValeur());
			}
		...
		if (f==Feve.F_MQ) {
			...
			if (this.Rentabilites(f, cours_de_f)==true && cours_de_f < prix_seuil_1) { //si le cours permet de faire au moins 10% de profit, on ne met en vente que les fèves 
				return this.getStockTotTimeTheo(Feve.F_MQ, (int)this.stepsVecuesPourBourseMQ.getValeur()); //MQ qui vont bientôt se déclasser en BQ
			}
			if (cours_de_f >= prix_seuil_1 && cours_de_f <= prix_seuil_2) { // si le prix est suffisemment élevé, on met en vente aussi les fèves de MQ qui ne sont pas proches d'être déclassées
				return (stock_mis_en_bourse(f)-this.getStockTotTimeTheo(Feve.F_MQ,(int)this.stepsVecuesPourBourseMQ.getValeur()))*(cours_de_f - prix_seuil_1)/(prix_seuil_2 - prix_seuil_1) + this.getStockTotTimeTheo(Feve.F_MQ, (int)this.stepsVecuesPourBourseMQ.getValeur());
			}
			if(cours_de_f >= prix_seuil_2) {
				return stock_mis_en_bourse(f);
			}
...
//					car elles ont rétrogradé en fève MQ BE proches de la destruction)
	...
	public Lot notificationVente(Feve f, double quantiteEnT, double coursEnEuroParT) {
		...
		if (f == Feve.F_MQ) { //
			...
			this.MQquantiteVendueBourse.setValeur(null, quantiteLivre, this.cryptogramme);
		}
		if (f == Feve.F_BQ)
